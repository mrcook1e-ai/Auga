...
public class Player : Humanoid
{
	...
	private void PlayerAttackInput(float dt)
	{
		...
		else
		{
			...
			if ((m_queuedAttackTimer > 0f || m_attackHold) && StartAttack(null, false))
			{
				m_queuedAttackTimer = 0f;
			}
			if ((m_queuedSecondAttackTimer > 0f || m_secondaryAttackHold) && StartAttack(null, true))
			{
				m_queuedSecondAttackTimer = 0f;
			}
		}
